...
app.post("/api/train-csv", upload.any(), async (req, res) => {
  try {
    ...
    if (!labelKey) throw new Error("Missing labelKey");

    const savedPath = saveUploadedFile(file);

    const st = readState();
    writeState({ ...st, activeCsvPath: savedPath });

    const result = await trainCsvNN({ csvPath: savedPath, labelKey });
    res.json({ ok: true, result, activeCsvPath: savedPath });
  } catch (e) {
...
app.post("/api/train-text", upload.any(), async (req, res) => {
  try {
    ...
    // allow optional params from UI (if not sent, use defaults)
    const result = await trainTextLSTM({
      txtPath: savedPath,
